...
> 1. 本地版本控制系统
>
>    ![本地版本控系统](./image/本地版本控制系统.jpg)
>
> 2. 集中式版本控制系统
>
>    ![集中式版本控制系统](./image/集中式版本控制系统.jpg)
>
> 3. 分布式版本控制系统
>
...
* 分类概观：
  ...
  >
  >    1. **已提交(*commited*)**：也可以称为“_未修改_”，与“_已修改_”相对应，表示某个已跟踪的文件目前没有修改，没必要暂存+提交。
  >
  >       别名：未修改
  >
  >    2. **已修改(*modified*)**：表示某个已跟踪文件已经被修改，待暂存+提交
  >
  >       别名：待暂存
  >
  >    3. **已暂存(*staged*)**：表示某个文件“曾”是已修改状态，并且经过暂存操作，现在待提交
  >
  >       别名：待提交
  >
...
* Git的基本工作流：
  ...
  > 示例说明：
  >
  > 1. 修改了a，当前工作目录中的a变成了$a^{`}$；b保持不变；
  > 2. 暂存$a^{`}$：将$a^{`}$暂存到暂存区中；由于b为修改，仍未b，所以没有被暂存；
  > 3. 提交暂存区中的修改到仓库：将暂存区中的$a^{`}$提交到Git仓库中，生成一次新的commit对象，称之为version_1；version_1中保存了&$a^{`}$与&b；最初的version_0中依旧保存&a与&b；
  >
  > 图示：
  >
  > ![Git基本工作流](./image/Git基本工作流.jpg)
  >
  > 图示二：加入未跟踪文件的描述
  >
  > ![Git基本工作流](./image/Git基本工作流2.jpg)
...
* ***当项目到达某个需要记录的状态时向仓库提交这些变更的快照***——有一个时间点，过早、过频繁的进行提交与一次性提交多个修改都是不赞成的；类似于函数的编写，一个单独的功能实体可以作为一次提交；

* 文件状态的生命周期

  ![文件状态的生命周期](./image/Git基本工作流2.jpg)
...
* `git add`
  * 上一小节中提到的**将修改添加到暂存区**或者说**将新blob对象暂存**功能
...
* `git rm --cached`
  ...
  副作用即为：删除`working copy`中的目标文件

* `git rm`可以使用通配符，即`.gitignore`中的通配符
...
* `git mv src.c new.c`命令
  ...
  > 当前的状态：
  >
  > R_ sr.c -> new.c（也就是说，git识别到了这次重命名，即使我们分开操作！！！）
...
* 两个功能：
  ...
  2. 上一次提交后，向暂存区添加了新的修改：允许你将当前暂存区中的修改合并到上一次提交中
...
* 将已暂存到暂存区中的修改撤销“add"操作，恢复到`已修改、未提交`的状态
...
* **提交对象（`commit object`）**：发起提交时，Git存储的是提交对象
  ...
  * 提交对象中包含**指向暂存区中快照的指针**；
  * 当`add`操作将文件添加到暂存区中时，实际上是将当前`working copy`中的对应文件的一份压缩版本放到了暂存区中；
  * 提交信息中包含：作者姓名、邮箱、已输入的提交信息、指向父提交的指针；

* **父提交**：初始提交没有父提交；一般一个提交对象有一个父提交或者多个父提交；

* **blob对象**：每个文件的每一次修改都会生成一个新的blob对象记录文件的全部内容（而不是差异）

  * blob对象保存在.git/object/目录下；目录名为该blob对象SHA-1校验和的前两位，目录下的（压缩）文件名为该blob对象SHA-1校验和的后续46位；

* **树对象**：类似于Linux文件系统实现，目录中保存文件名与对应blob对象的对应关系；

  * blob对象用校验和来标记
  * 树对象的校验和是如何计算的呢？每次提交一定会生成新的树对象吗？（暂时看起来是这样的）
...
* 一次操作流程：
  ...
  * 每次`git add`操作会新增一个blob（有时候如果一个文件已经被add过一次，则不会新增）保存在.git/object目录下
...
* 图示：
  ...
  * 一次提交所有相关的对象
    ...
    ![一次提交所有相关的对象](./image/一次提交所有相关对象.jpg)

  * 多次提交形成的链

    ![多次提交形成链](./image/多次提交形成链.jpg)
...
* 切换分支的真正含义：
  ...
  * 这么一来，checkout的意义就呼之欲出了:)
* `git checkout testing`
* 切换分支失败的情形：
  * 切换分支时，当前分支上有未添加到暂存区的修改——未添加到暂存区可能面临丢失的风险
  * 切换分支时，暂存区中有与切换到的目标分支上的内容冲突——20190428暂存区中的内容必须commit，否则也无法切换分支
* 如何避免失败？
  * 在切换分支时，保证clean
  * 储藏、修订提交
...
* 冲突的解决
   ...
   * 文件中冲突的形式
     ...
     >>>>>>> iss53:index.html
     # 上半部分是HEAD指向的分支的文件对应部位的内容
     # 下半部分是分支iss53分支的文件对应部位的内容
     # 上半部分HEAD指向的分支事实上就是我们当前所处的分支
     # 可以将“合并”看做是：将iss53的修改在HEAD（当前分支）重现
     ```
...
* 分支列举：`git branch`
  ...
  * 已合并分支列举：`git branch --merged`
    ...
    > 已合入到当前分支的分支
    >
    > 如果当前分支是master，那么这种已合入主线的分支一般就可以直接删除了:)

  * 未合并分支列举：`git branch --no-merged`

    > 为何如当前分支的分支
    >
    > 如果当前分支是master，那么这种未合入主线的分支请不要轻易删除，当然可以通过如下命令删除：
    >
    > `git branch -D xxx`
...
* 示例：
  ...
  * 更新*远程仓库*在本地的内容：`git fetch origin`
    ...
    > ![更新远程仓库](./image/更新远程仓库.jpg)
    >
    > 1. 我们再本地分支上进行了一些开发（注意，远程分支仍然没有发生移动）
    >
    > ![添加其他远程仓库](./image/添加其他远程仓库.jpg)
    >
...
* 上述第三种操作
  ...
  * 图示：
    ...
    ![rebase_onto操作之前](./image/rebase_onto操作之前.jpg)
...
* 为什么需要一个远程仓库？
  ...
  * 理论上来讲，可以将别人的本地仓库作为“远程仓库”，进行拉取或推送；

    但是，容易产生混乱

    别人的计算机必须一直保持在线

  * 推荐方式：*建立一个你与协作者可以共同访问的中间仓库，大家都向该仓库推送数据，也都从中拉取数据*
...
* 第5章会详细讲解有关分布式版本控制环境下的相关操作

  * 也就是管理远程仓库、推送、拉取等相关工作流的详细指导

* 裸仓库（bare repository）：没有对应的工作目录的Git仓库

  * 由于这种仓库只作为协作点（collaboration point），因此检出数据到磁盘上没有任何意义
...
* 优点：
  ...
  * 简单易用，可以使用已有的文件权限和网络访问权限；只需要把仓库拷贝到大家都可以访问到的共享目录中；

  * 当需要快速获取其他人的仓库数据时，可以通过下面命令而无需其上传以后，我们再去fetch
...
* 智能HTTP协议
  ...
  * 可以被设置为同git://一样的匿名访问，也可以设置为SSH一样的验证访问，且仅需要一个URL就可以实现；
...
* 将仓库拷贝到服务器`/srv/git/`目录下
  ...
  # 这样，其他用户就可以通过以下命令clone你的仓库——前提是其他用户具有该服务器的ssh权限
...
*   简化版的Git数据模型
    ...
    ![简化版Git数据模型](./image/简化版Git数据模型.jpg)
...
*   引用/数据对象/树对象/提交对象之间的关系：
    ...
    ![各种对象间的关系图解](.\image\各种对象间的关系图解.JPG)
